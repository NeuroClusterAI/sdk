...
async def main():
    ...
    if not agent_id:
        agent = await neurocluster_client.Agent.create(
            name="Generic Agent",
        ...
        agent = await neurocluster_client.Agent.get(agent_id)
        await agent.update(allowed_tools=["get_weather"])

    # Setup the thread
    thread_id = kv.get("thread_id")
    if not thread_id:
        thread = await neurocluster_client.Thread.create()
        kv.set("thread_id", thread._thread_id)
    else:
        thread = await neurocluster_client.Thread.get(thread_id)
